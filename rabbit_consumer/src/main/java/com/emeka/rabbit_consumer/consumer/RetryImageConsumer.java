...
public class RetryImageConsumer {
	...
	public void listenForImageConsumer(Message message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag)
	...
	public void listenForVectorConsumer(Message message,Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag ){
		try {
			var p = objectMapper.readValue(message.getBody(), Picture.class);
			// process the image
			if (p.getSize() > 9000) {
